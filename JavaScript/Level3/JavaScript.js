...
const createQuestion = () => {
    do {
        qNum1 = Math.floor(Math.random() * 25 + 1);
...
const createNumbers = () => {
    ...
    let val2;
    while (true) {
        val1 = Math.floor(Math.random() * 25 + 1);
        val2 = Math.floor(Math.random() * 9 + 2);


        if (val1 < val2 || (val1 % val2 != 0) || (qNum1 / qNum2) == (val1 / val2))
            continue;
        else {
            for (let i = 0; i < wrongNumbers.length; i++) {
                if ((parseInt(val1 / val2)) == wrongNumbers[i])
                    continue;
            }
            wrongNumbers.push(Math.floor(val1 / val2));
        }
        if (wrongNumbers.length == 8)
            break;
    }

    numbersValues[0].innerHTML = `${Math.floor(qNum1 / qNum2)}`;
    numberDivs[0].appendChild(numbersValues[0]);
    for (var i = 0; i < wrongNumbers.length; i++) {
        numbersValues[i + 1].innerHTML = `${wrongNumbers[i]}`;
        numberDivs[i + 1].appendChild(numbersValues[i + 1]);
    }
}

window.addEventListener("load", createQuestion());

window.addEventListener("load", createNumbers());
...
const data = JSON.parse(localStorage.getItem("totalScore"));
const scoreValue = {
    scoreNum: data,
    hitsNum: 0,
    missNum: 0
...
if (saveFile) {
    ...
    scoreValue.hitsNum = `${JSON.parse(localStorage.getItem("lv3Hit"))}`;
    scoreValue.missNum = `${JSON.parse(localStorage.getItem("lv3Miss"))}`;
}
...
const correct = () => {
    ...
    let qNum2 = Math.floor(Math.random() * 9 + 2);
...
const wrong = () => {
    ...
    if (scoreValue.missNum == 6) {
        ...
        localStorage.setItem("lv3Hit", JSON.stringify(scoreValue.hitsNum));
        localStorage.setItem("lv3Miss", JSON.stringify(scoreValue.missNum));
